...
# Check if /usr/bin/convert-im7.q16 from ImageMagic is available
convert -version > /dev/null 2>&1
if [ $? -ne 0 ]; then
  echo "Task-Distributor requires the command line tool /usr/bin/convert-im7.q16 from ImageMagic!"; exit 1
fi
...
do
  ssh -i ~/.ssh/id_ed25519 ${USERNAME_SSH}@${HOSTS_ARRAY[$i]} ${REMOTE_SCRIPT} ${NUM_NODES} ${IMG_PATH} ${IMG_FILE} +FN +W${IMG_WIDTH} +H${IMG_HEIGHT} +O${OUTPUT_DIR} +SR${START} +ER${END} ${IMAGE_PARTS_PATH} ${LOCKFILE} &
...
if [ "$NUM_NODES" -eq 1 ] ; then
  ...
  if cp ${IMAGE_PARTS_PATH}/*.png /tmp/output_${IMG_WIDTH}x${IMG_HEIGHT}_${NUM_NODES}_nodes_`date +%Y_%m_%d_%H:%M:%S`.png ; then
