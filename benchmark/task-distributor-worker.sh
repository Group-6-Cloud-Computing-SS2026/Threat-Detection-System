...
# Extract numeric dimensions from +Wxxxx and +Hyyyy strings
W_VAL=$(echo $WIDTH | cut -c 3-)
H_VAL=$(echo $HEIGHT | cut -c 3-)

# Use physical storage if resolution is higher than 3200x2400
...
if [ $NUM_NODES -eq 1 ] ; then
  ...
  TERM=dumb /usr/bin/povray +I${IMG_PATH}/${IMG_FILE} ${FN} ${WIDTH} ${HEIGHT} +O${TARGET_DIR}/${BASENAME}.png ${START} ${END} 1>/dev/null 2>/tmp/povraymessages
