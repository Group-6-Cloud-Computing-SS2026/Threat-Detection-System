...
|---|---|---|---|
| **1st** | **Md. Forman Ullah Sajib** | 2, 3, 4, 5 | Backend–frontend wiring, authentication, live camera stream, detection data, deployment |
...
### 6.1 — Project Initialisation & Tooling
...
### 6.3 — Authenticated Application Pages
...
### 7.1 — GraphsPage (Scaling Law Results)
...
### 2.3 — Authenticated Application Pages
...
- `Logo` / wordmark SVGs — brand assets
- `HomeSidebar` — collapsible mobile navigation with toggle
- `GraphsPage` — Chart.js integration with horizontal drag-to-scroll tables, colour palette, and responsive layout
- `OperationsPage` — sectioned tables with consistent error states and responsiveness
- Custom **Nacelle** font family loaded via `@font-face`
- PWA manifest and updated favicon
...
sequenceDiagram
    ...
    LoginPage->>FastAPI: POST /api/v1/auth/login
    FastAPI-->>LoginPage: JWT token + username
    LoginPage->>AuthContext: setAuth({ token, username })
    AuthContext->>localStorage: Persist tds-auth-state
    AuthContext-->>User: Redirect to /home (useEffect guard)
    User->>LoginPage: Visit /login while authenticated
    LoginPage-->>User: Redirect to /home (already authed)
```

- `AuthContext` provides `authState` and `handleLogin` / `handleLogout` globally via React Context
- `ProtectedRoute` guards all authenticated pages — unauthenticated users are redirected to `/login`
- **Login tab** can be pre-selected via URL query param (`?tab=login`)
- Registration supports role selection: `viewer`, `operator`, `admin`
...
### 4.1 — GraphsPage (Scaling Law Results)
- Displays **Amdahl's Law** and **Gustafson's Law** benchmark results from the cluster benchmarking task
- Line charts rendered with **Chart.js** with custom colour palettes, tooltip styling, and typed options
- Tables with multi-line headers, column width adjustments, and **horizontal drag-to-scroll** on mobile
- Responsive layout switching between grid and flex depending on viewport
...
### 4.4 — SettingsPage
- API Base URL configuration (passed down to data-layer hooks)
- Flex-based responsive layout with updated button styles
...
This section covers the core data pipeline that makes the dashboard operational: connecting the browser to the live backend, camera stream, and detection database.

### 5.1 — Problem Statement
The frontend needed to display two simultaneous live data streams from the cluster:

1. **Camera frames** — base64-encoded JPEGs published by the Pi camera to MQTT, relayed through the FastAPI backend at up to 10 fps. Standard REST polling would be too slow and too expensive.
2. **Detection events** — structured JSON records written to PostgreSQL when YOLO classifies a threat. These need to be surfaced in the browser with linked images from MinIO.

### 5.2 — Live Camera Feed (WebSocket)

```mermaid
sequenceDiagram
    participant Pi as Edge Node (Pi Camera)
    participant MQTT as Mosquitto Broker
    participant API as FastAPI Backend
    participant WS as /stream/ws
    participant Browser

    Pi->>MQTT: Publish base64 JPEG frame
    MQTT->>API: Deliver to all subscribers (broadcast topic)
    API->>WS: Relay frame to connected WebSocket clients
    WS-->>Browser: JSON { image: "<base64>", timestamp: "..." }
    Browser->>Browser: Render <img>, compute FPS & latency
```

- WebSocket connects to `/api/v1/stream/ws?token=<jwt>` on page load
- Each frame payload contains a `base64` JPEG and a `timestamp`
- **Latency** = `Date.now()` − frame timestamp (end-to-end from camera to browser render)
- **FPS** = rolling average over the last 10 inter-frame intervals
- Auto-reconnects after 5 seconds on disconnect; status badge shows `Live` / `Connecting` / `Offline`

### 5.3 — Live Detection Panel

- Polls `GET /api/v1/detections/recent?limit=8` every **10 seconds**
- Auto-refresh toggle (pause without losing the current data)
- Detection cards display: event type, severity badge (`critical` → `low`), confidence percentage, timestamp
- **Expand a card** to see:
  - Raw YOLO detection JSON and metadata
  - Annotated preview image loaded from MinIO via `/images/{id}/download`
  - All stored images for that event fetched from `/api/v1/detections/{id}`
- Summary chip-row aggregates counts by event type across loaded events

### 5.4 — Event Search & Filter

- Filter fields: `event_type`, `severity`, `sensor_id`, `acknowledged`, `start_time`, `end_time`, `skip`, `limit`
- Uses the backend's `PaginatedResponse<T>` schema — total count, offset, and current page are all displayed
- Results use the same expandable card component as the live panel
- Filters build into a `URLSearchParams` query string sent to `GET /api/v1/detections`

### 5.5 — API Utility Layer
...
async function apiFetch(path: string, options?: RequestInit) {
  ...
  if (!response.ok) throw new Error(`${response.status} ${message}`)
  return response.json()
}
```

- Single `apiFetch` wrapper attaches the JWT, handles errors, and resolves to typed JSON
- API Base URL is read from `localStorage` at startup so the same build targets any cluster endpoint without rebuilding
...
```
Stage 1 (node:20-alpine)   →   npm install + vite build  →  /dist
Stage 2 (nginx:1.25-alpine) →  copy /dist + nginx.conf   →  :80
```
...
The `Upgrade` and `Connection` headers are required to allow the WebSocket handshake to pass through Nginx to the backend.

### Kubernetes Manifest (`frontend.yaml`)

- **Deployment**: 1 replica (stateless — all state lives in the backend)
- **Service**: ClusterIP on port 80
- **Ingress**: Traefik routes `http://192.168.1.50/` to the frontend pod; `/api/*` paths forward to the backend
