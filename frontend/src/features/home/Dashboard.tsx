...
export default function Dashboard() {
    ...
    async function fetchRecentEvents() {
        ...
        setLiveError(null);
        try {
            const data = (await apiFetch(apiBaseUrl, token, "/detections/recent?limit=8")) as DetectionEvent[];
            setLiveEvents(
                data.map((item) => ({
                    ...item,
    ...
    async function loadDetectionDetail(eventId: string, listName: "live" | "search") {
        ...
        try {
            const detail = (await apiFetch(apiBaseUrl, token, `/detections/${eventId}`)) as DetectionDetail;
            ...
            const resolvedPreviewUrl = buildImageUrl(detail.preview_image_url, apiBaseUrl);
    ...
    return (
        <div className="space-y-8">
            <section className="grid gap-6 lg:grid-cols-[1fr_auto]">
                <div>
                    ...
                    <p className="mb-5 max-w-2xl text-brand-alabaster-grey-600">
                        Poll the backend for the latest detections, search previous records with filters, and open
                        stored images from each event.
                    </p>
                    <div className="flex flex-wrap gap-3">
                        <Button variant="secondary" size="sm" type="button" onClick={clearPersonFilter}>
                            Clear person filter
                        </Button>
                        <Button variant="secondary" size="sm" href="/camera_stream.html" target="_blank" rel="noreferrer">
                            Open camera preview
                        </Button>
                        <Button variant="secondary" size="sm" href={`${apiBaseUrl.replace(/\/api\/v1\/?$/, "")}/docs`} target="_blank" rel="noreferrer">
                            Open Swagger API
                        </Button>
                    </div>
                </div>
            ...
            <section className="rounded-2xl border border-brand-carbon-black-800 bg-brand-carbon-black-900/60 p-5">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 className="font-nacelle text-lg font-semibold text-brand-alabaster-grey-100">Connection</h2>
                        <p className="text-sm text-brand-alabaster-grey-600">
                            Signed in as <strong className="text-brand-alabaster-grey-300">{auth?.username}</strong>. Update the
                            API base URL if needed.
                        </p>
                    </div>
                    <Button variant="secondary" size="sm" type="button" onClick={() => void fetchRecentEvents()}>
                        Refresh now
                    </Button>
                </div>
                ...
                <div className="flex flex-wrap items-end gap-3">
                    ...
                    <Button type="button" size="sm" onClick={() => saveApiBaseUrl(draftApiBaseUrl)}>
                        Save connection
                    </Button>
                    <Button variant="secondary" size="sm" type="button" onClick={() => setAutoRefresh((value) => !value)}>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                <section className="rounded-2xl border border-brand-carbon-black-800 bg-brand-carbon-black-900/60 p-5 xl:col-span-2">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 className="font-nacelle text-lg font-semibold text-brand-alabaster-grey-100">Camera preview</h2>
                            <p className="text-sm text-brand-alabaster-grey-600">Embedded live stream from camera_stream.html.</p>
                        </div>
                        <Button variant="secondary" size="sm" href="/camera_stream.html" target="_blank" rel="noreferrer">
                            Open in new tab
                        </Button>
                    </div>
                    <div className="overflow-hidden rounded-xl border border-brand-carbon-black-800 bg-brand-pitch-black-500">
                        <iframe
                ...
                <section className="rounded-2xl border border-brand-carbon-black-800 bg-brand-carbon-black-900/60 p-5">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 className="font-nacelle text-lg font-semibold text-brand-alabaster-grey-100">Live detections</h2>
                            <p className="text-sm text-brand-alabaster-grey-600">Polling /detections/recent every 5 seconds.</p>
                        </div>
                        <span
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                        {liveSummary.length > 0 ? (
                            liveSummary.map(([label, count]) => (
                                <span key={label} className="rounded-full bg-brand-carbon-black-800 px-2.5 py-1 text-xs text-brand-alabaster-grey-300">
                                    {label}: {count}
                                </span>
                            ))
                        ) : (
                            <span className="rounded-full bg-brand-carbon-black-800 px-2.5 py-1 text-xs text-brand-alabaster-grey-600">
                                No detections yet
                            </span>
                        )}
                    </div>
                ...
                <section className="rounded-2xl border border-brand-carbon-black-800 bg-brand-carbon-black-900/60 p-5">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 className="font-nacelle text-lg font-semibold text-brand-alabaster-grey-100">Previous detections</h2>
                            <p className="text-sm text-brand-alabaster-grey-600">
                                Filter the historical list by event type, severity, sensor, time, and status.
                            </p>
                        </div>
                    ...
                    <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                        ...
                        <label className={labelClass}>
                            ...
                            <select
                                ...
                                onChange={(event) => setQuery((current) => ({ ...current, severity: event.target.value }))}
                            >
                        ...
                        <label className={labelClass}>
                            ...
                            <input
                                ...
                                onChange={(event) => setQuery((current) => ({ ...current, sensorId: event.target.value }))}
                                placeholder="UUID"
                            />
                        </label>
                        <label className={labelClass}>
                            ...
                            <select
                                ...
                                onChange={(event) => setQuery((current) => ({ ...current, acknowledged: event.target.value }))}
                            >
                                <option value="">Any</option>
                        ...
                        <label className={labelClass}>
                            ...
                            <input
                                ...
                                className={inputClass}
                                value={query.skip}
                                onChange={(event) => setQuery((current) => ({ ...current, skip: event.target.value }))}
                            />
                        </label>
                        <label className={labelClass}>
                    ...
                    <div className="mb-3 flex justify-between text-xs text-brand-alabaster-grey-600">
...
function StatCard({ label, value }: { label: string; value: string }) {
    return (
        <div className="min-w-32 rounded-xl border border-brand-carbon-black-800 bg-brand-carbon-black-900/60 px-4 py-3">
            <span className="block text-xs text-brand-alabaster-grey-600">{label}</span>
            <strong className="font-nacelle text-xl font-semibold text-brand-alabaster-grey-100">{value}</strong>
        </div>
    );
}

function Notice({ tone = "info", children }: { tone?: "info" | "error"; children: string }) {
    const toneClass =
        tone === "error"
            ? "bg-brand-brick-red-950 text-brand-brick-red-300"
            : "bg-brand-carbon-black-800 text-brand-alabaster-grey-400";
    return <div className={`mb-4 rounded-lg px-3 py-2 text-sm ${toneClass}`}>{children}</div>;
}
